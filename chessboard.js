...
;(function() {
  ...
  function validAnimationSpeed (speed) {
    return Number.isInteger(speed) && speed >= 0 || speed == 'fast' || speed == 'slow'
  }
  ...
  function expandConfig() {
    ...
    // default piece theme
    if (!cfg.pieceTheme) {
      cfg.pieceTheme = "images/chesspieces/newspaper/{piece}.png";
    }

    // default color board theme
    if (!cfg.boardTheme) {
      cfg.boardTheme = ["#cccccc", "#928b8b"];
    }

    // default animation speeds
    if (!validAnimationSpeed(cfg.appearSpeed)) {
      cfg.appearSpeed = 200;
    }
    if (!validAnimationSpeed(cfg.moveSpeed)) {
      cfg.moveSpeed = 200;
    }
    if (!validAnimationSpeed(cfg.snapbackSpeed)) {
      cfg.snapbackSpeed = 50;
    }
    if (!validAnimationSpeed(cfg.snapSpeed)) {
      cfg.snapSpeed = 25;
    }
    if (!validAnimationSpeed(cfg.trashSpeed)) {
      cfg.trashSpeed = 100;
    }
  ...
  */
  ...
  function calculatePositionFromMoves (position, moves) {
    function calculateExplosionSquares(explosq) {
      const file = explosq[0]
      const rank = parseInt(explosq[1])
      if (file == 'a') {
        var explofiles = ['a', 'b']
      }
      else if (file == 'h') {
        ...
        var explofiles = [String.fromCharCode(file.charCodeAt(0)-1),
                         file,
      ...
      if (rank == 1) {
        ...
        var exploranks = [rank-1, rank, rank+1]
      }
      let explosquares = []
      explofiles.forEach((file) => {
        exploranks.forEach((rank) => {
          explosquares.push(file + String(rank))
        })
      })
      return explosquares
    }
    ...
    for (var i in moves) {
      ...
      if (newPosition[moves[i]]) {
        const explosquares = calculateExplosionSquares(moves[i])
        explosquares.forEach((sq) => {
          if (newPosition[sq] && newPosition[sq][1] != "P") {
